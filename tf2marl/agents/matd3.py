...
class MATD3Agent(AbstractAgent):
    def __init__(
        ...
        self.critic_1 = MADDPGCriticNetwork(
            2, num_units, lr, obs_shape_n, act_shape_n, act_type, agent_index
        )
        self.critic_1_target = MADDPGCriticNetwork(
            2, num_units, lr, obs_shape_n, act_shape_n, act_type, agent_index
        )
        self.critic_1_target.model.set_weights(self.critic_1.model.get_weights())

        self.critic_2 = MADDPGCriticNetwork(
            2, num_units, lr, obs_shape_n, act_shape_n, act_type, agent_index
        )
        self.critic_2_target = MADDPGCriticNetwork(
            2, num_units, lr, obs_shape_n, act_shape_n, act_type, agent_index
        )
    ...
    def update(self, agents, step):
        ...
        critic_outputs[0] = self.critic_1_target.predict(next_obs_n, target_act_next)[
            :, 0
        ]
        critic_outputs[1] = self.critic_2_target.predict(next_obs_n, target_act_next)[
            :, 0
        ]
        ...
        td_loss[0] = self.critic_1.train_step(
            obs_n, acts_n, q_train_target, weights
        ).numpy()[:, 0]
        td_loss[1] = self.critic_2.train_step(
            obs_n, acts_n, q_train_target, weights
        ).numpy()[:, 0]
        ...
        if self.update_counter % self.policy_update_freq == 0:  # delayed policy updates
            ...
            policy_loss = None
        self._run.log_scalar(
            "agent_{}.train.q_loss0".format(self.agent_index), np.mean(td_loss[0]), step
        )
        self._run.log_scalar(
            "agent_{}.train.q_loss1".format(self.agent_index), np.mean(td_loss[1]), step
        )
